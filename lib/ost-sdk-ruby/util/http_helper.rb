    ...
    class HTTPHelper
      ...
      def send_post_request(endpoint, request_params)
        perform_and_handle_exceptions('u_hh_1', 'POST request failed') do
          ...
          if @api_spec
            return OSTSdk::Util::Result.success({data: {request_uri: uri.to_s, request_type: 'POST', request_params: query_string}})
          else
            result = http.post(uri.path, URI.escape(query_string))
      ...
      def send_get_request(endpoint, request_params)
        perform_and_handle_exceptions('u_hh_2', 'GET request Failed') do
          ...
          r_params = base_params.merge(request_params)
          query_string = Rack::Utils.build_nested_query(r_params)
          raw_url = get_api_url(endpoint) + "?" + URI.escape(query_string)
          uri = URI(raw_url)
          if @api_spec
            return OSTSdk::Util::Result.success({data: {request_uri: uri.to_s.split("?")[0], request_type: 'GET', request_params: query_string}})
          else
            result = {}
      ...
      def generate_signature(string_to_sign)
        ...
        puts "generate_signature- " + signature
        signature
      end
